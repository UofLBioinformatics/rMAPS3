...
##  CD[7]=[[0]*exonNum]*eP;
  ...
  for aa in exons: ## process each exon in this group
    ...
    eEle=aa.split(':');
    chr = eEle[0]; strand = eEle[1]; eS= int(eEle[2])+1; eE=int(eEle[3]);
    ueE=int(exons[aa][6]); ## upstream exon end, 1-base
    deS=int(exons[aa][7])+1; ## downstream exon start, 1-base

    uee=[ueE-exonLen+1,ueE];
    uis=[ueE+1,ueE+intronLen];
    uie=[eS-intronLen, eS-1];
    tes=[eS,eS+exonLen-1];
    tee=[eE-exonLen+1,eE];
    dis=[eE+1,eE+intronLen];
    die=[deS-intronLen,deS-1];
    des=[deS,deS+exonLen-1]; 

    if strand=='-': ## for negative strand
      uee=[deS,deS+exonLen-1];
      uis=[deS-intronLen,deS-1];
      uie=[eE+1,eE+intronLen];
      tes=[eE-exonLen+1,eE];
      tee=[eS,eS+exonLen-1];
      dis=[eS-intronLen, eS-1];

    #1st region (upstream exon end)
    countPeaks(chr,strand,uee,rVal[0],winSize,step,CD[0],exonIndex);
    countPeaks(chr,strand,uis,rVal[1],winSize,step,CD[1],exonIndex);
    countPeaks(chr,strand,uie,rVal[2],winSize,step,CD[2],exonIndex);
    countPeaks(chr,strand,tes,rVal[3],winSize,step,CD[3],exonIndex);
    countPeaks(chr,strand,tee,rVal[4],winSize,step,CD[4],exonIndex);
    countPeaks(chr,strand,dis,rVal[5],winSize,step,CD[5],exonIndex);
    countPeaks(chr,strand,die,rVal[6],winSize,step,CD[6],exonIndex);
    countPeaks(chr,strand,des,rVal[7],winSize,step,CD[7],exonIndex);     
...
def drawNode(c,eH,eW,iW,indent,gap,sGap,scale): ## draw node
  ...
  #eH=20; eW=50; iW=200; indent=5; gap=10; sGap=20; 
  ...
  #targetExon=path.rect((eW+iW+gap+iW)*scale,0,(eW+iW+gap+iW+gap+gap+eW)*scale,eH*scale);
...
def drawBox(c,mpv,eH,eW,iW,indent,gap,sGap,scale,el,il,maxNegP, separate=False): ## draw plot box
  ...
  myMax=int(mpv+1);
  yLab=['0.0', str(myMax*0.25), str(myMax*0.5), str(myMax*0.75), str(float(myMax))];
  ...
  xS=0;xE=xS+bW; yl_offset=5; xl_offset=15;
  ...
  ## y-label  
  c.text((xS-yl_offset-yl_offset*8)*scale,(boxY+bH/2.0)*scale,"Average CLIP Peak Height",[titleLab,text.halign.boxcenter,trafo.rotate(90)]);
  #x-axis
  ...
  ## Title and legends
  c.text((xE+gap)*scale, (boxY+bH+55)*scale, "RNA MAP FOR "+ proteinLabel, [titleLab, text.halign.boxcenter]);

  c.stroke(path.line((xE+gap*2)*scale, (boxY+bH+20)*scale,(xE+gap*3)*scale, (boxY+bH+20)*scale),[upColor,mls]); 
  c.text((xE+gap*4)*scale, (boxY+bH+15)*scale, "Upregulated", [upColor,largeLab]);
  c.stroke(path.line((xE+gap*3+iW/2)*scale, (boxY+bH+20)*scale,(xE+gap*3+iW/2+gap)*scale, (boxY+bH+20)*scale),[dnColor,mls]); 
  c.text((xE+gap*3+iW/2+gap+gap)*scale, (boxY+bH+15)*scale, "Downregulated", [dnColor,largeLab]);
  c.stroke(path.line((xE+gap*2+iW+gap+gap+gap)*scale, (boxY+bH+20)*scale,(xE+gap*2+iW+gap+gap+gap+gap)*scale,(boxY+bH+20)*scale),[bgColor,bgmls]); 
  c.text((xE+gap*2+iW+gap+gap+gap+gap+gap)*scale, (boxY+bH+15)*scale, "Background", [bgColor,largeLab]);

  c.stroke(path.line((xE+gap*2)*scale, (boxY+bH+40)*scale,(xE+gap*3)*scale, (boxY+bH+40)*scale),[upColor,ldash]);
  c.text((xE+gap*4)*scale, (boxY+bH+35)*scale, "-log(pVal) up vs. bg", [upColor,largeLab]);
  c.stroke(path.line((xE+6*gap+iW*2/3)*scale, (boxY+bH+40)*scale,(xE+6*gap+iW*2/3+gap)*scale, (boxY+bH+40)*scale),[dnColor,ldash]);
  c.text((xE+8*gap+iW*2/3)*scale, (boxY+bH+35)*scale, "-log(pVal) dn vs. bg", [dnColor,largeLab]);
  ...
  # yp-axis
  xE = xE+eW;
  c.text((xE-6*yl_offset-gap)*scale,boxY*scale,ypLab[0],ypLabAtt);
  c.text((xE-6*yl_offset-gap)*scale,(boxY+bH/4.0)*scale,ypLab[1],ypLabAtt);
  c.text((xE-6*yl_offset-gap)*scale,(boxY+bH/2.0)*scale,ypLab[2],ypLabAtt);
  c.text((xE-6*yl_offset-gap)*scale,(boxY+3*bH/4.0)*scale,ypLab[3],ypLabAtt);
  c.text((xE-6*yl_offset-gap)*scale,(boxY+bH)*scale,ypLab[4],ypLabAtt);
  ## y-label
  c.text((xE+yl_offset*3)*scale,(boxY+bH/2.0)*scale,"Negative log10(pValue)",[titleLab,text.halign.boxcenter,trafo.rotate(270)]);
...
def drawAcutalPlot(c,dp,mpv,eH,eW,iW,gap,sGap,scale,wl,sl,nPP,mNP, separate=False): ## draw plots now..nPP is negativePvaluePoins, mNP is maximumNegPval
  ...
  ## 1st and 2nd region
  xS=0;
  points_up,points_dn,points_bg=plotRegions(dp[0],dp[1],xS,boxY,bH,eW,iW,mpv,scale);
