...
event_type = sys.argv[14].upper() if len(sys.argv) > 14 else 'RI'
...
def countPeaksIntron(ch,st,region,rv,ws,ss,countDist,ei): ## fill up rv and countDist with proper values, exon index was passed for wilcoxon test
  ...
  if st=='+': ### positive strand
    rStart=region[0];
    lenreg=region[1]-region[0]+1;
    for i in range(lenreg): ## we already know how many times we need to do
      Newi=int((float(i) / lenreg)*intrlen);
      ...
      for tk in td: ## for all non-duplicate peak counts, add it to the position
        if yees:
          ...
          if done[Newi]==0:
            for j in range(lasti+1,Newi+1,1):
              tC[j]+=td[tk];
          done[Newi] += 1;
        else:
          if done[Newi] == 0:
...
def processExons(exons,peaks,CD,intronLen=250,exonLen=50,winSize=50,step=10): ## now process exons in each group
  ...
  rVal[0]=[0]*eP;rVal[1]=[0]*eP;rVal[2]=[0]*iP;rVal[3]=[0]*eP;
  rVal[4]=[0]*eP;
...
##  CD[7]=[[0]*exonNum]*eP;
  ...
  for epep in range(eP): ## for 0,3,4,7
    for epindex in [0,1,3,4]: ## that has eP points
      CD[epindex][epep]=[0]*exonNum;
  for ipip in range(iP): ## for 1,2,5,6
    for ipindex in [2]: ## that has iP points
  ...
  for aa in exons: ## process each exon in this group
    ...
    r1 = [upES, upES + exonLen - 1];
    r2 = [upEE - exonLen + 1, upEE - 1];
    r3 = [upEE,dnES - 1];
    r4 = [dnES , dnES + exonLen- 1];
    r5 = [dnEE- exonLen + 1, dnEE ];
    ...
    if strand=='-': ## for negative strand
      ...
      r5 = [upES, upES + exonLen - 1];
      r4 = [upEE - exonLen + 1, upEE - 1];
      r3 = [upEE, dnES - 1];
      r2 = [dnES, dnES + exonLen - 1];
      r1 = [dnEE - exonLen + 1, dnEE];
    ...
    countPeaksIntron(chr,strand,r3,rVal[2],winSize,step,CD[2],exonIndex);
...
def drawNode(c,eH,eW,iW,indent,gap,sGap,scale): ## draw node
  ...
  upstreamExon = path.path(path.moveto(0, 0), path.lineto((2 * eW + gap + gap) * scale, 0),
                           path.lineto((2 * eW + gap + gap) * scale, scale * eH), path.lineto(0, scale * eH),
                           path.lineto(0, 0));
  intron = path.line((2 * eW + gap + gap) * scale, scale * eH / 2, scale * ((2 * eW + gap + gap) + iW), scale * eH / 2);

  newX = (2 * eW + gap + gap) + iW;  ## newX
  downstreamExon = path.rect(newX * scale, 0, (eW + gap + gap + eW) * scale, eH * scale)
...
def drawBox(c,mpv,eH,eW,iW,indent,gap,sGap,scale,el,il,maxNegP, separate=False): ## draw plot box
  ...
  c.stroke(path.line((xE + gap + iW + gap * 15) * scale, (boxY + bH + 25) * scale, (xE + gap + iW + gap * 16) * scale,
  ...
  c.text((xE + gap + iW + gap * 17) * scale, (boxY + bH + 20) * scale, "Background(" + str(nb) + ")",
         [bgColor, largeLab]);
  ...
  xS = xE + 2 * gap;
  xE = xS + eW + iW + eW;
  rect2 = path.rect(xS * scale, boxY * scale, (eW + iW + eW) * scale, bH * scale);
  ...
  r3_ss = path.line((xS + eW + iW) * scale, boxY * scale, (xS + eW + iW) * scale, (boxY + boxHeight) * scale);

  # x-axis
  c.text(xS * scale, (boxY - xl_offset) * scale, xLab_2[0], [largeLab]);
  c.text((xS + eW) * scale, (boxY - xl_offset) * scale, xLab_2[1], [largeLab]);
  c.text((xS + eW + iW) * scale, (boxY - xl_offset) * scale, xLab_2[1], [largeLab]);

  c.text((xS + eW + iW + eW) * scale, (boxY - xl_offset) * scale, xLab_2[2], [largeLab, text.halign.boxright]);
...
def drawAcutalPlot(c,dp,mpv,eH,eW,iW,gap,sGap,scale,wl,sl,nPP,mNP, separate=False): ## draw plots now..nPP is negativePvaluePoins, mNP is maximumNegPval
  ...
  ## 3rd and 4th region
  xS = xS + eW + 2 * gap;
  points_up, points_dn, points_bg = plotRegions(dp[1], dp[2], xS, boxY, bH, eW, iW, mpv, scale);
  path_up = fillUpPath(points_up);
  path_dn = fillUpPath(points_dn);
  path_bg = fillUpPath(points_bg);
  c.stroke(path_up, [upColor, mls]);
  c.stroke(path_dn, [dnColor, mls]);
  c.stroke(path_bg, [bgColor, bgmls]);
  points_pup, points_pdn = plotRegions(nPP[1], nPP[2], xS, boxY, bH, eW, iW, mNP, scale, 2);  ## return 2 values
  path_pup = fillUpPath(points_pup);
  path_pdn = fillUpPath(points_pdn);
  c.stroke(path_pup, [upColor, ldash]);
  c.stroke(path_pdn, [dnColor, ldash]);
  ...
  ## 5th and 6th region
  xS = xS + eW + iW;
  points_up, points_dn, points_bg = plotRegions(dp[3], dp[3], xS, boxY, bH, eW, iW, mpv, scale, 3, 0);
  path_up = fillUpPath(points_up);
  path_dn = fillUpPath(points_dn);
  path_bg = fillUpPath(points_bg);
  c.stroke(path_up, [upColor, mls]);
  c.stroke(path_dn, [dnColor, mls]);
  c.stroke(path_bg, [bgColor, bgmls]);
  points_pup, points_pdn = plotRegions(nPP[3], nPP[3], xS, boxY, bH, eW, iW, mNP, scale, 2, 0);  ## return 2 values
  path_pup = fillUpPath(points_pup);
  path_pdn = fillUpPath(points_pdn);
  c.stroke(path_pup, [upColor, ldash]);
  c.stroke(path_pdn, [dnColor, ldash]);
  ...
  ## 7th and8 th region
  xS = xS + eW + 2 * gap;
  points_up, points_dn, points_bg = plotRegions(dp[4], dp[4], xS, boxY, bH, eW, eW, mpv, scale, 3, 0);
  path_up = fillUpPath(points_up);
  path_dn = fillUpPath(points_dn);
  path_bg = fillUpPath(points_bg);
  c.stroke(path_up, [upColor, mls]);
  c.stroke(path_dn, [dnColor, mls]);
  c.stroke(path_bg, [bgColor, bgmls]);
  points_pup, points_pdn = plotRegions(nPP[4], nPP[4], xS, boxY, bH, eW, eW, mNP, scale, 2, 0);  ## return 2 values
  path_pup = fillUpPath(points_pup);
  path_pdn = fillUpPath(points_pdn);
  c.stroke(path_pup, [upColor, ldash]);
  c.stroke(path_pdn, [dnColor, ldash]);
