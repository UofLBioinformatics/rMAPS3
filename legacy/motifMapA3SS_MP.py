...
def getFasta(t):  ## get fasta for the given exon group
    ...
    for line in f:  ## process each event
        ...
        chr = ele[0]
        strand = '+'
        r5 = [int(ele[4]), int(ele[3])]
        r4 = [int(ele[2]), int(ele[4])]
        ...
        if ele[1] == '-' or ele[1] == '-1':  ## negative strand
            r5 = [int(ele[2]), int(ele[5])]
            r4 = [int(ele[5]), int(ele[3])]
...
def drawNode(c, eH, eW, iW, indent, gap, sGap, scale):  ## draw node
    ...
    intron_1 = path.line((2 * eW + 2 * gap) * scale, scale * eH / 2,
                         (2 * eW + 2 * gap + iW) * scale, scale * eH / 2)
    divider_1 = [
        path.line(
            (2 * eW + 2 * gap + iW - sGap) * scale, (eH / 2 - gap) * scale,
            ((2 * eW + 2 * gap + iW) + sGap) * scale, (eH / 2 + gap) * scale),
        path.line(((2 * eW + 2 * gap + iW - sGap) + gap) * scale,
                  (eH / 2 - gap) * scale,
                  ((2 * eW + 2 * gap + iW + sGap) + gap) * scale,
                  (eH / 2 + gap) * scale)
    ]
    intron_2 = path.line((2 * eW + 2 * gap + iW + gap) * scale, scale * eH / 2,
                         (2 * eW + 2 * gap + iW + gap + iW) * scale,
                         scale * eH / 2)
    spliceregion = path.path(
        path.moveto((2 * eW + 2 * gap + iW + gap + iW) * scale, 0),
        path.lineto(
            (2 * eW + 2 * gap + iW + gap + iW + 2 * eW + 2 * gap) * scale, 0),
        path.lineto(
            (2 * eW + 2 * gap + iW + gap + iW + 2 * eW + 2 * gap) * scale,
            scale * eH),
