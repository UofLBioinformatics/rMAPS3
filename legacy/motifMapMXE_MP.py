...
def setup_runtime():
    ...
    parser.add_argument('-r',
                        ...
                        dest='rMATS',
                        required=True,
                        help='an rMATS output file from MXE event')
    parser.add_argument('-mi',
                        '--miso',
                        dest='miso',
                        required=True,
                        help='an miso output file from MXE event')
    parser.add_argument(
        '-u',
        '--up',
        dest='up',
        required=True,
        help=
        'a tab delimited file containing upregulated exons with the following headers. GeneID geneSymbol chr strand 1stExonStart_0base	1stExonEnd 2ndExonStart_0base 2ndExonEnd upstreamES upstreamEE downstreamES downstreamEE'
    )
    parser.add_argument(
        '-d',
        '--down',
        ...
        required=True,
        help=
        'a tab delimited file containing downregulated exons with the following headers. GeneID geneSymbol chr strand 1stExonStart_0base 1stExonEnd 2ndExonStart_0base 2ndExonEnd upstreamES upstreamEE downstreamES downstreamEE'
    )
    parser.add_argument(
        '-b',
        '--background',
        ...
        required=True,
        help=
        'a tab delimited file containing background exons with the following headers. GeneID geneSymbol chr strand 1stExonStart_0base 1stExonEnd 2ndExonStart_0base 2ndExonEnd upstreamES upstreamEE downstreamES downstreamEE'
    )
    ...
    region = [
        "UpstreamExon", "FirstIntron", "SecondIntron", "FirstExon", "thirdIntron",
        "fourthIntron", "SecondExon", "fifthIntron", "sixthIntron",
        "downstreamExon"
    ]
    rName = [
    ...
    if miso != "NA":  ## got miso input here
        rMATS = tempPath + '/converted.rMATS.mxe.txt'
        convcmd = ['perl', binPath + '/miso2rMATS.MXE.pl', '1', '100', miso, rMATS]
...
def makeInputFiles(
    ...
    header = 'chr\tstrand\tfirstExonStart_0base\tfirstExonEnd\tsecondExonStart_0base\tsecondExonEnd\tupstreamES\tupstreamEE\tdownstreamES\tdownstreamES'
    uFile.write(header + '\n')
    dFile.write(header + '\n')
    bFile.write(header + '\n')
...
def getFasta(t):  ## get fasta for the given exon group
    ...
    for line in f:  ## process each event
        ...
        r2 = [int(ele[7]), int(ele[7]) + iLen + wLen]
        r3 = [int(ele[2]) - iLen - wLen, int(ele[2])]
        r4 = [int(ele[2]), int(ele[3])]
        r5 = [int(ele[3]), int(ele[3]) + iLen + wLen]
        r6 = [int(ele[4]) - iLen - wLen, int(ele[4])]
        r7 = [int(ele[4]), int(ele[5])]
        r8 = [int(ele[5]), int(ele[5]) + iLen + wLen]
        r9 = [int(ele[8]) - iLen - wLen, int(ele[8])]
        r10 = [int(ele[8]), int(ele[9])]

        if ele[1] == '-' or ele[1] == '-1':  ## negative strand
            r10 = [int(ele[6]), int(ele[7])]
            r9 = [int(ele[7]), int(ele[7]) + iLen + wLen]
            r8 = [int(ele[2]) - iLen - wLen, int(ele[2])]
            r7 = [int(ele[2]), int(ele[3])]
            r6 = [int(ele[3]), int(ele[3]) + iLen + wLen]
            r5 = [int(ele[4]) - iLen - wLen, int(ele[4])]
            r4 = [int(ele[4]), int(ele[5])]
            r3 = [int(ele[5]), int(ele[5]) + iLen + wLen]
            r2 = [int(ele[8]) - iLen - wLen, int(ele[8])]
            r1 = [int(ele[8]), int(ele[9])]
            strand = '-'
                    ...
                    if len(cc) > 0:  ## mapped somewhere
                        ...
                        for pInd in cc:  ## index
                            ...
                            if pInd >= (max(iLen, eLen) + wLen) or nInd < -(
                                ...
                                if rg == "UpstreamExon":  ## do it both way
                                    ...
                                    if nInd >= (-iLen - wLen):  ## valid
                                        tempPosition = range(
                                            max(0, inInd - wLen + 1) // sLen,
                                            min(iP, inInd // sLen + 1))
                                        for tptp in tempPosition:
                                            cdist[jj][2][tptp][exonNum] += 1
                                elif rg == "FirstExon":  ## do it negative way
                                ...
                                elif rg == "thirdIntron":  ## do it both way
                                    if (pInd < (iLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(iP, pInd // sLen) - 1)
                                ...
                                elif rg == "fourthIntron":  ### do it positive way
                                    if nInd >= (-iLen - wLen):  ## valid
                                        tempPosition = range(
                                            max(0, inInd - wLen + 1) // sLen,
                                            min(iP, inInd // sLen + 1))
                                ...
                                elif rg == "SecondExon":  ## do it negative way
                                    if (pInd < (eLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(eP, pInd // sLen) - 1)
                                    ...
                                    if nInd >= (-eLen - wLen):  ## valid
                                        tempPosition = range(
                                            ...
                                            min(eP, enInd // sLen + 1))
                                        for tptp in tempPosition:
                                            cdist[jj][8][tptp][exonNum] += 1
                                elif rg == "fifthIntron":  ## do it both way
                                    if (pInd < (iLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(iP, pInd // sLen) - 1)
                                        for tptp in tempPosition:
                                            cdist[jj][9][tptp][exonNum] += 1

                                elif rg == "sixthIntron":  ### do it positive way
                                    if nInd >= (-iLen - wLen):  ## valid
                                        tempPosition = range(
                                            max(0, inInd - wLen + 1) // sLen,
                                            min(iP, inInd // sLen + 1))
                                        for tptp in tempPosition:
                                            cdist[jj][10][tptp][exonNum] += 1
                                elif rg == "downstreamExon":  ## do it negative way
                                    if (pInd < (eLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(eP, pInd // sLen) - 1)
                                        for tptp in tempPosition:
                                            cdist[jj][11][tptp][exonNum] += 1
...
def drawNode(c, eH, eW, iW, indent, gap, sGap, scale):  ## draw node
    ...
    firstExon = path.rect((eW + iW + gap + iW) * scale, 0,
    ...
    secondExon = path.rect((newX + iW + gap + iW) * scale, 0,
                           (eW + gap + gap + eW) * scale, eH * scale)
    newX = newX + iW + gap + iW + eW + gap + gap + eW
    intron_5 = path.line(newX * scale, (eH / 2) * scale, (newX + iW) * scale,
                         scale * eH / 2)
    divider_3 = [
        path.line((newX + iW - sGap) * scale, (eH / 2 - gap) * scale,
                  (newX + iW + sGap) * scale, (eH / 2 + gap) * scale),
    ...
    intron_6 = path.line((newX + iW + gap) * scale, scale * eH / 2,
                         (newX + iW + gap + iW) * scale, (eH / 2) * scale)
...
def drawBox(c,
    ...
    drawutils.draw_x_axis_segment(x_axis_canvas, scale, xS, boxY, xLab_1_3,
                                  units_per_bp)
    ...
    drawutils.draw_x_axis_segment(x_axis_canvas, scale, xS, boxY, xLab_2_4,
                                  units_per_bp)
    ...
    xS = xE + 2 * divider_gap
    xE = xS + bW
    rect5, r5_line, r5_ss = drawutils.boxes(xS, bW, scale, boxY, boxHeight,
                                            exon_width)
    ...
    xS = xE + divider_gap
    xE = xS + bW
    rect6, r6_line, r6_ss = drawutils.boxes(xS, bW, scale, boxY, boxHeight,
                                            intron_width)
...
def drawAcutalPlot(
    ...
    xS = xS + iW + eW + gap + gap
    points_up, points_dn, points_bg = plotRegions(dp[8], dp[9], xS, boxY, bH,
                                                  eW, iW, mpv, scale)
    path_up = fillUpPath(points_up)
    path_dn = fillUpPath(points_dn)
    path_bg = fillUpPath(points_bg)
    ...
    c.stroke(path_dn, [dnColor, mls])
    c.stroke(path_bg, [bgColor, bgmls])
    points_pup, points_pdn = plotRegions(nPP[8], nPP[9], xS, boxY, bH, eW, iW,
                                         mNP, scale, 2)
    path_pup = fillUpPath(points_pup)
    path_pdn = fillUpPath(points_pdn)
    c.stroke(path_pup, [upColor, ldash, p_transform])
    ...
    xS = xS + eW + iW + gap
    points_up, points_dn, points_bg = plotRegions(dp[10], dp[11], xS, boxY, bH,
                                                  iW, eW, mpv, scale)
    path_up = fillUpPath(points_up)
    path_dn = fillUpPath(points_dn)
    path_bg = fillUpPath(points_bg)
    ...
    c.stroke(path_dn, [dnColor, mls])
    c.stroke(path_bg, [bgColor, bgmls])
    points_pup, points_pdn = plotRegions(nPP[10], nPP[11], xS, boxY, bH, iW,
                                         eW, mNP, scale, 2)
    path_pup = fillUpPath(points_pup)
    path_pdn = fillUpPath(points_pdn)
    c.stroke(path_pup, [upColor, ldash, p_transform])
...
def plotMotifs_finale(mapName,
    ...
    pdf_path = mapsPath + '/' + 'ME.' + mapName + '.pdf'
    png_path = mapsPath + '/' + 'ME.' + mapName + '.png'
    pdf_ok, png_ok = drawutils.export_canvas_outputs(
        canv, pdf_path, png_path, png_resolution=80, logger=logging)
    if not pdf_ok:
...
def plotMotifs(
    ...
    drawPoints = []
    negPvalPoints = []
    maxPointValue = 0.00000000000000000000000000000000000000000000001
    maxNegPval = 0.000000000000000000000000000000000000000000000000000000001
    ...
    d.append(val)
    logging.debug("Done drawing motif map. Please find " + mapName +
...
def minPvalueOut(exonType):
    ...
    for fN in fileList:
        ...
        for eachLine in pValData:
            ...
            if tmpStr[0] == "R12":
                pValR12Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R11":
                pValR11Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R10":
                pValR10Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R9":
                pValR09Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R8":
                pValR08Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R7":
                pValR07Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R6":
                pValR06Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R5":
                pValR05Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R4":
                pValR04Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R3":
                pValR03Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R2":
                pValR02Chunk.append(float(tmpStr[-1]))
            elif tmpStr[0] == "R1":
                pValR01Chunk.append(float(tmpStr[-1]))
            else:
                pass
        pValR01Chunk.sort()
        pValR02Chunk.sort()
        ...
        pValList.append([
            ...
            bucket_min(pValR08Chunk), bucket_min(pValR09Chunk), bucket_min(pValR10Chunk), bucket_min(pValR11Chunk),
            bucket_min(pValR12Chunk)
        ])
    pValList.sort(key=operator.itemgetter(2))
    fileOpen = open(outPath + "/" + resultFile, "w")
    fileOpen.write("RBP\tR1\tR2\tR3\tR4\tR5\tR6\tR7\tR8\tR9\tR10\tR11\tR12\n")
    for sortedPvalue in pValList:
        fileOpen.write(sortedPvalue[0] + '\t' + str(sortedPvalue[1]) + '\t' +
                       ...
                       '\t' + str(sortedPvalue[7]) + '\t' +
                       str(sortedPvalue[8]) + '\t' + str(sortedPvalue[9]) +
                       '\t' + str(sortedPvalue[10]) + '\t' +
                       str(sortedPvalue[11]) + '\t' + str(sortedPvalue[12]) +
                       '\n')
    fileOpen.close()
    ...
    try:
        ...
        for i in range(len(d)):
            ...
            maybe_plot_motif_map(mapname, maxPointValue, maxNegPval, drawPoints,
                                 negPvalPoints, args.separate, outPath, 'MXE')
        ...
        if args.motif != 'NA':
            ...
            for i in range(len(d2)):
                ...
                maybe_plot_motif_map(mapname, maxPointValue, maxNegPval,
                                     drawPoints, negPvalPoints, False, outPath, 'MXE')
