...
def setup_runtime():
    ...
    parser.add_argument('-r',
                        ...
                        dest='rMATS',
                        required=True,
                        help='an rMATS output file from A5SS event')
    parser.add_argument('-mi',
                        '--miso',
                        dest='miso',
                        required=True,
                        help='an miso output file from A5SS event')
    parser.add_argument(
        '-u',
        '--up',
        dest='up',
    ...
    region = [
        "LongExon", "SplicingRegion", "FirstIntron", "FlankinfExonIntron",
        "FlankingExon"
    ]
    rName = [
    ...
    if miso != "NA":  ## got miso input here
        rMATS = tempPath + '/converted.rMATS.A5SS.txt'
        convcmd = ['perl', binPath + '/miso2rMATS.A5SS.pl', '1', '100', miso, rMATS]
...
def getFasta(t):  ## get fasta for the given exon group
    ...
    for line in f:  ## process each event
        ...
        chr = ele[0]
        strand = '+'
        r1 = [int(ele[2]), int(ele[5])]
        r2 = [int(ele[5]), int(ele[3])]
        r3 = [int(ele[3]), int(ele[3]) + iLen + wLen]
        r4 = [int(ele[6]) - iLen - wLen, int(ele[6])]
        r5 = [int(ele[6]), int(ele[7])]

        if ele[1] == '-' or ele[1] == '-1':  ## negative strand
            r1 = [int(ele[4]), int(ele[3])]
            r2 = [int(ele[2]), int(ele[4])]
            r3 = [int(ele[2]) - iLen - wLen, int(ele[2])]
            r4 = [int(ele[7]), int(ele[7]) + iLen + wLen]
            r5 = [int(ele[6]), int(ele[7])]
            strand = '-'
                    ...
                    if len(cc) > 0:  ## mapped somewhere
                        ...
                        for pInd in cc:  ## index
                            ...
                            if pInd >= (max(iLen, eLen) + wLen) or nInd < -(
                                ...
                                elif rg == "SplicingRegion":  ## do it both way
                                    if (pInd < (eLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(eP, pInd // sLen) - 1)
                                ...
                                elif rg == "FirstIntron":  ### do it positive way
                                    if pInd >= (iLen + wLen):
                                        continue
                                    tempPosition = range(
                                        max(0, pInd - wLen + 1) // sLen,
                                ...
                                elif rg == "FlankinfExonIntron":  ## do it negative way
                                    if nInd < (-iLen - wLen):
                                        continue
                                    tempPosition = range(
                                ...
                                elif rg == "FlankingExon":  ## do it both way
                                    if (pInd < (eLen + wLen)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen + 1) // sLen,
                                            min(eP, pInd // sLen) - 1)
...
def plotMotifs_finale(mapName,
    ...
    pdf_path = mapsPath + '/' + 'A5SS.' + mapName + '.pdf'
    png_path = mapsPath + '/' + 'A5SS.' + mapName + '.png'
    ...
    try:
        ...
        for i in range(len(d)):
            ...
            maybe_plot_motif_map(mapname, maxPointValue, maxNegPval, drawPoints,
                                 negPvalPoints, args.separate, outPath, 'A5SS')
        ...
        if args.motif != 'NA':
            ...
            for i in range(len(d2)):
                ...
                maybe_plot_motif_map(mapname, maxPointValue, maxNegPval,
                                     drawPoints, negPvalPoints, False, outPath, 'A5SS')
