...
def setup_runtime():
    ...
    parser.add_argument('-r',
                        ...
                        dest='rMATS',
                        required=True,
                        help='an rMATS output file from RI event')
    parser.add_argument('-mi',
                        '--miso',
                        dest='miso',
                        required=True,
                        help='an miso output file from RI event')
    parser.add_argument(
        '-u',
        '--up',
        dest='up',
        required=True,
        help=
        'a tab delimited file containing upregulated exons with the following headers. GeneID	geneSymbol	chr	strand	riExonStart_0base	riExonEnd	upstreamES	upstreamEE	downstreamES	downstreamEE'
    )
    parser.add_argument(
        '-d',
        '--down',
        ...
        required=True,
        help=
        'a tab delimited file containing downregulated exons with the following headers. GeneID	geneSymbol	chr	strand	riExonStart_0base	riExonEnd	upstreamES	upstreamEE	downstreamES	downstreamEE'
    )
    parser.add_argument(
        '-b',
        '--background',
        ...
        required=True,
        help=
        'a tab delimited file containing background exons with the following headers. GeneID	geneSymbol	chr	strand	riExonStart_0base	riExonEnd	upstreamES	upstreamEE	downstreamES	downstreamEE'
    )
    ...
    region = ["UpstreamExon", "Intron", "downstreamExon"]
    rName = [
    ...
    if miso != "NA":  ## got miso input here
        rMATS = tempPath + '/converted.rMATS.ri.txt'
        convcmd = ['perl', binPath + '/miso2rMATS.RI.pl', '1', '100', miso, rMATS]
...
def makeInputFiles(
    ...
    header = 'chr\tstrand\triExonStart_0base\triExonEnd\tupstreamES\tupstreamEE\tdownstreamES\tdownstreamEE'
    uFile.write(header + '\n')
    dFile.write(header + '\n')
    bFile.write(header + '\n')
...
def getFasta(t):  ## get fasta for the given exon group
    ...
    for line in f:  ## process each event
        ...
        r2 = [int(ele[5]), int(ele[6])]
        r3 = [int(ele[6]), int(ele[7])]

        if ele[1] == '-' or ele[1] == '-1':  ## negative strand
            r3 = [int(ele[4]), int(ele[5])]
            r2 = [int(ele[5]), int(ele[6])]
                    ...
                    if len(cc) > 0:  ## mapped somewhere
                        ...
                        for pInd in cc:  ## index
                            ...
                            if pInd >= (max(iLen, eLen) + wLen) or nInd < -(
                                ...
                                elif rg == "Intron":  ## do it both way
                                    pInd = int((float(pInd) / seqLen) * iLen)
                                    wLen1 = int((float(wLen) / seqLen) * iLen)
                                    if (pInd < (iLen + wLen1)):  ## valid
                                        tempPosition = range(
                                            max(0, pInd - wLen1 + 1) // sLen,
                                            min(iP, pInd // sLen) - 1)
                                        for tptp in tempPosition:
...
def drawNode(c, eH, eW, iW, indent, gap, sGap, scale):  ## draw node
    ...
    intron = path.line((2 * eW + gap + gap) * scale, scale * eH / 2,
                       scale * ((2 * eW + gap + gap) + iW), scale * eH / 2)

    newX = (2 * eW + gap + gap) + iW
    downstreamExon = path.rect(newX * scale, 0, (eW + gap + gap + eW) * scale,
                               eH * scale)
...
def drawBox(c,
    ...
    xS = xE + 2 * divider_gap
    xE = xS + 2 * exon_width + intron_width
    rect2, r2_line, r2_ss = drawutils.boxes(xS, 2 * exon_width + intron_width,
                                            scale, boxY, boxHeight, exon_width)
    r3_ss = path.line((xS + exon_width + intron_width) * scale, boxY * scale,
                      (xS + exon_width + intron_width) * scale,
                      (boxY + boxHeight) * scale)

    xl_offset = 23
    x_axis_canvas.text(xS * scale, (boxY - xl_offset) * scale, str(xLab_2[0]),
                       [largeLab])
    x_axis_canvas.text((xS + exon_width) * scale, (boxY - xl_offset) * scale,
                       str(xLab_2[1]), [largeLab])
    x_axis_canvas.text((xS + exon_width + intron_width) * scale,
                       (boxY - xl_offset) * scale, str(xLab_2[1]), [largeLab])

    x_axis_canvas.text((xS + 2 * exon_width + intron_width) * scale,
                       (boxY - xl_offset) * scale, str(xLab_2[2]),
...
def drawAcutalPlot(
    c, dp, mpv, eH, eW, iW, gap, sGap, scale, wl, sl, nPP, mNP, separate=False
    ...
    xS = xS + eW + iW
    points_up, points_dn, points_bg = plotRegions(dp[3], dp[3], xS, boxY, bH,
                                                  eW, iW, mpv, scale, 3, 0)
    path_up = fillUpPath(points_up)
    path_dn = fillUpPath(points_dn)
    path_bg = fillUpPath(points_bg)
    ...
    c.stroke(path_dn, [dnColor, mls])
    c.stroke(path_bg, [bgColor, bgmls])
    points_pup, points_pdn = plotRegions(nPP[3], nPP[3], xS, boxY, bH, eW, iW,
                                         mNP, scale, 2, 0)
    path_pup = fillUpPath(points_pup)
    path_pdn = fillUpPath(points_pdn)
    c.stroke(path_pup, [upColor, ldash, p_transform])
    ...
    xS = xS + eW + 2 * gap
    points_up, points_dn, points_bg = plotRegions(dp[4], dp[4], xS, boxY, bH,
                                                  eW, eW, mpv, scale, 3, 0)
    path_up = fillUpPath(points_up)
    path_dn = fillUpPath(points_dn)
    path_bg = fillUpPath(points_bg)
    ...
    c.stroke(path_dn, [dnColor, mls])
    c.stroke(path_bg, [bgColor, bgmls])
    points_pup, points_pdn = plotRegions(nPP[4], nPP[4], xS, boxY, bH, eW, eW,
                                         mNP, scale, 2, 0)
    path_pup = fillUpPath(points_pup)
    path_pdn = fillUpPath(points_pdn)
    c.stroke(path_pup, [upColor, ldash, p_transform])
...
def plotMotifs_finale(mapName, maxPointValue, maxNegPval, drawPoints,
                      negPvalPoints, separate=False):
    canv = canvas.canvas()
    exonHeight = 30
    exonWidth = eLen
    intronWidth = iLen
    ...
    pdf_path = mapsPath + '/' + 'RI.' + mapName + '.pdf'
    png_path = mapsPath + '/' + 'RI.' + mapName + '.png'
...
def plotMotifs(
    ...
    drawPoints = []
    negPvalPoints = []
    maxPointValue = 0.00000000000000000000000000000000000000000000000001
    maxNegPval = 0.0000000000000000000000000000000000000000000000001
...
def minPvalueOut(exonType):
    ...
    for fN in fileList:
        ...
        pValList.append([
            ...
            bucket_min(pValR04Chunk), bucket_min(pValR05Chunk)
        ])
    pValList.sort(key=operator.itemgetter(2))
    fileOpen = open(outPath + "/" + resultFile, "w")
    fileOpen.write("RBP\tR1\tR2\tR3\tR4\tR5\n")
    ...
    try:
        ...
        for i in range(len(d)):
            ...
            maybe_plot_motif_map(mapname, maxPointValue, maxNegPval, drawPoints,
                                 negPvalPoints, args.separate, outPath, 'RI')
        ...
        if args.motif != 'NA':
            ...
            for i in range(len(d2)):
                ...
                maybe_plot_motif_map(mapname, maxPointValue, maxNegPval,
                                     drawPoints, negPvalPoints, args.separate, outPath, 'RI')
